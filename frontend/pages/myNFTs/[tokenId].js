...
const TokenDetail = () => {
  ...
  useEffect(() => {
    if(mintContract !== null) {
      ...
      axios.get(url)
        .then(res => {
          setToken(res.data);
          setCurrentTokenFile(res.data.tokenId)
        })
        .catch(err => {
  ...
  const setCurrentTokenFile = async (tid) => {
    let loop = true;
    let offset = 0;
    let mintedTokenFiles = []

    while(loop) {
      const config = {
        method: 'get',
        url: `https://api.pinata.cloud/data/pinList?metadata[name]=HHoYM&status=pinned&pageLimit=1000&pageOffset=${offset}`,
        headers: {
      ...
      mintedTokenFiles = [ ...mintedTokenFiles, ...res.data.rows ];
      if(res.data.rows.length < 1000) loop = false;
      offset += 1000
    }

    const currentTokenFile = mintedTokenFiles.find(item => item?.metadata.keyvalues.tokenId === tid);
    console.log(mintedTokenFiles, mintedTokenFiles.map(item => item?.metadata.keyvalues.tokenId), tokenId, currentTokenFile);

    if(!!!currentTokenFile) swal('Cannot find this token\'s CID');

    if(currentTokenFile && currentTokenFile?.metadata?.keyvalues?.profileAddress !== 'empty') {
      axios.get(mintContract.base_uri + currentTokenFile.metadata.keyvalues.profileAddress)
        .then(res => setProfile({...res.data}));
    }

    setMetadata(currentTokenFile?.metadata);
    if(currentTokenFile) setTokenFile(currentTokenFile);
                    ...
                    <div className='p-5 text-white'>
                      <div className='mb-4'>
                        <div className='flex w-full justify-between '>
                          <div className={'flex'}>
                            <ArrowBackIcon className='cursor-pointer' onClick={() => router.push('/myNFTs')}></ArrowBackIcon>
                            <p className={'mx-2 glowing-text-small'}>
                              {`Hodly Hero #${tokenId}`}
                            </p>
                          </div>
                          <a className={`download-glb mx-2 w-fit px-7 py-2`} href={token.animation_url}>Download GLB</a>
                        </div>
                      </div>
                      {/*<div className="row">*/}
                      {/*  {token.description}*/}
                      {/*</div>*/}
                      <div className="flex row mt-6" style={{ alignItems: 'center' }}>
                        <model-viewer
                          src={token.animation_url}
                          ios-src=""
                          poster={mintContract.base_uri + token.resources[1].uri}
                      ...
                      <Typography>
                        <TableContainer component={Paper} className='!bg-cyan-200/10 mt-6 p-6'>
                          <Table size={'small'} aria-label="simple table">
                            ...
                            <div className='button cursor-pointer p-6 mt-10 m-auto w-full'  onClick={() => setOpenProfile(true)}>
                              { metadata?.keyvalues?.profileAddress === 'empty' ? "Create Backstory" : "Edit Backstory" }
                            </div>
                          </Table>
                        </TableContainer>
                      </Typography>
